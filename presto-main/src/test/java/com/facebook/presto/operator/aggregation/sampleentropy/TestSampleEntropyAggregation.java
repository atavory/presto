        ...
        extends AbstractTestAggregationFunction
{
    ...
    protected TestSampleEntropyAggregation(String method)
    {
        this.method = method;
    }
    ...
    public Block[] getSequenceBlocks(int start, int length)
    {
        ...
        for (int weight = 1; weight < 3; ++weight) {
            for (int i = start; i < start + length; i++) {
                BIGINT.writeLong(bucketCountBlockBuilder, TestSampleEntropyAggregation.NUM_BINS);
                DOUBLE.writeDouble(minBlockBuilder, 0.0);
                DOUBLE.writeDouble(maxBlockBuilder, Double.valueOf(TestSampleEntropyAggregation.NUM_BINS));
            ...
            ArrayList<Double> weights)
    {
        for (int weight = 1; weight < 3; ++weight) {
            for (int i = start; i < start + length; ++i) {
                final int bin = Math.max(Math.min(i, TestSampleEntropyAggregation.NUM_BINS - 1), 0);

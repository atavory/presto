        ...
        extends AbstractTestAggregationFunction
{
        ...
        if (arg1 != null) {
            aggregationFunction = functionManager.getAggregateFunctionImplementation(
                    functionManager.lookupFunction(
                            TestDifferentialEntropyAggregation.functionName,
    ...
    protected TestDifferentialEntropyAggregation(String method, Double arg0, Double arg1)
    {
        this.method = method;
        this.arg0 = arg0;
        this.arg1 = arg1;
    ...
    public Block[] getSequenceBlocks(int start, int length)
    {
        ...
        for (int weight = 1; weight < 3; ++weight) {
            for (int i = start; i < start + length; i++) {
                BIGINT.writeLong(sizeBlockBuilder, TestDifferentialEntropyAggregation.SIZE);
                DOUBLE.writeDouble(
                        sampleBlockBuilder,
                        Double.valueOf(
                                clip((double) i, 0.0, (double) TestDifferentialEntropyAggregation.SIZE)));
                VARCHAR.writeSlice(methodBlockBuilder, wrappedBuffer(this.method.getBytes()));
            ...
            ArrayList<Double> weights)
    {
        for (int weight = 1; weight < 3; ++weight) {
            for (int i = start; i < start + length; ++i) {
                final int bin = clip(i, 0, TestDifferentialEntropyAggregation.SIZE - 1);

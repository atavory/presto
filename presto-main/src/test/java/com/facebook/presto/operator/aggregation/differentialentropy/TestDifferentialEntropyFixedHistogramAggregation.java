...
public abstract class TestDifferentialEntropyFixedHistogramAggregation
        extends TestDifferentialEntropyAggregation
{
    private static final double min = 0;
    private static final double max = (double) TestDifferentialEntropyAggregation.SIZE;

    protected TestDifferentialEntropyFixedHistogramAggregation(String method)
    {
        super(
                method,
                TestDifferentialEntropyFixedHistogramAggregation.min,
                TestDifferentialEntropyFixedHistogramAggregation.max);
    }
    ...
    protected double getEntropyFromSamplesAndWeights(ArrayList<Double> samples, ArrayList<Double> weights)
    {
        ...
        final double rawEntropy = bucketWeights.values().stream()
                ...
                .sum() / Math.log(2.0);
        final double width =
                (TestDifferentialEntropyFixedHistogramAggregation.max - TestDifferentialEntropyFixedHistogramAggregation.min) /
                        TestDifferentialEntropyAggregation.SIZE;
